...
export default function PharmacyPurchase() {
  ...
  const handleEditFromRegister = async (purchase) => {
    try {
      // switch to GRN tab
      setTab(0);
      ...
      setFormData((prev) => ({
        ...prev,
        grnNumber: purchase.grnNumber || prev.grnNumber,
      ...
      // try to map supplier name to id
      const matched = suppliers.find(
        (s) => s.supplierName?.trim() === (purchase.supplierName || "").trim(),
      );
      ...
      const loadedItems = res.data.map((it) => ({
        medicineId: it.medicineId || "",
        medicineName: it.medicineName || "",
        dosageType: it.dosageType || "",
        batchNo: it.batchNo || "",
  ...
  const saveGRN = async () => {
    ...
    try {
      ...
      const validItems = itemsWithAmount.filter(
        (item) =>
          item.medicineId &&
          item.medicineName &&
          item.batchNo &&
      ...
      if (editPurchaseId) {
        await api.put(`/api/pharmacy-purchase/${editPurchaseId}`, payload);
        ...
        // leave inventory sync to user if needed; or sync minimal changes
      ...
      // notify purchase register to reload
      setRefreshKey((k) => k + 1);
        ...
        <Box>
          <Paper
            sx={{
              ...
              color: "#fff",
              background:
                "linear-gradient(125deg, #0f2f6f, #2563eb 58%, #0891b2)",
                ...
                <Table sx={{ minWidth: 1350 }}>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell sx={{ fontWeight: "bold", minWidth: 90 }}>
                        HSN
                      </TableCell>
                      <TableCell sx={{ fontWeight: "bold", minWidth: 90 }}>
                        Amount
                      ...
                      <TableRow key={index}>
                          ...
                          <Autocomplete
                            ...
                            getOptionLabel={(option) => {
                              const code = option.itemCode
                                ? `[${option.itemCode}] `
                                : "";
                              return `${code}${getMedicineLabel(option) || option.medicineName || ""}`;
                            }}
                            value={
                              medicines.find(
                                (medicine) =>
                              ...
                              (item.medicineName
                                ? { medicineName: item.medicineName, dosageType: item.dosageType || "" }
                                : null)
                            }
                            onChange={(event, value) =>
                              selectMedicine(index, value)
                            }
                            renderInput={(params) => (
                              <TextField
                                {...params}
                        ...
                        <TableCell>
                          <Typography
                            ...
                            sx={{ minWidth: 80 }}
                          >
                            ₹{computeLineAmount(item).toFixed(2)}
                          </Typography>
                        </TableCell>
